...
class MixinSync:
    ...
    def _sync_skill_template(
        self, source_client: str, *, families: Optional[List[str]] = None
    ) -> Dict[str, object]:
        registry = self.load_registry()
        clients = self.load_clients()
        self._require_client(source_client, clients)
        family_filter = set(families or [])
        published_skill_ids = list(
            clients["clients"][source_client].get("published_skill_ids", [])
        )
        template_families = []
        seen_families = set()
        ...
        for family, members in sorted(self._family_members(registry).items()):
            if family_filter and family not in family_filter:
                continue
            if any(
                member.get("client_overrides", {}).get(source_client) == "disabled"
                    ...
                    template_families.append(
                        {
                            ...
                            "description": self._family_display_skill(members).get(
                                "description", ""
                            ),
                        }
                    )
        ...
        for family in sorted(family_filter):
            if family not in seen_families:
                ignored_families.append(
                    {
                        "conflict_family": family,
                        "reason": "源客户端当前没有已发布或显式禁用该 family",
                    }
                )
        return {
            "source_client": source_client,
            "generated_at": utc_now(),
            "published_family_count": len(
                [item for item in template_families if item["mode"] == "prefer"]
            ),
            "disabled_family_count": len(
                [item for item in template_families if item["mode"] == "disabled"]
            ),
            "families": sorted(
                template_families, key=lambda item: item["conflict_family"]
            ),
    ...
    def _target_skill_sync_preview(
        self, registry: Dict, source_template: Dict[str, object], target_client: str
    ) -> Dict[str, object]:
        actions = []
        summary = {
            "prefer_exact": 0,
            ...
            "noop": 0,
            "unresolved_family": 0,
            "unavailable_family": 0,
        }
        for family_item in source_template.get("families", []):
            family = family_item["conflict_family"]
            visible_members = self._family_members_visible_to_client(
                registry, target_client, family
            )
            current_override = self._family_override_value(
                [
                    skill
                    for skill in registry["skills"].values()
                    if skill["conflict_family"] == family
                ],
                target_client,
            )
                ...
                actions.append(
                    {
                        ...
                        "action": action_type,
                        "mode": "disabled",
                        "reason": "源客户端对该 family 使用显式禁用模板"
                        if action_type != "noop"
                        else "目标客户端已是 disabled",
                    }
                )
                continue
            ...
            exact_match = next(
                (
                    ...
                    if member["skill_id"] == source_skill_id
                ),
                None,
            )
            if exact_match:
                ...
                action_type = (
                    "prefer_exact" if current_override != preferred_value else "noop"
                )
                summary[action_type] += 1
                actions.append(
                    {
                        ...
                        "reason": "目标客户端可见同一 skill_id"
                        if action_type != "noop"
                        else "目标客户端已优先该 skill_id",
                    }
                )
    ...
    def _mcp_sync_preview(
        self, source_client: str, target_client: str
    ) -> Dict[str, object]:
        source_payload = self.mcp_list(source_client)
        target_payload = self.mcp_list(target_client)
        if source_payload["source_status"] != "ok" or not source_payload.get(
            "writable"
        ):
            return {
        ...
        if target_payload["source_status"] != "ok" or not target_payload.get(
            "writable"
        ):
            return {
                "client": target_client,
        ...
        source_entries = {
            ...
            for entry in source_payload.get("root_servers", [])
        }
        target_entries = {
            entry["name"]: dict(entry)
            for entry in target_payload.get("root_servers", [])
        }
        actions = []
        counts = {"add": 0, "update": 0, "noop": 0}
                ...
                actions.append(
                    {
                        "server_name": name,
                        "action": "add",
                        "reason": "目标客户端不存在该 server",
                    }
                )
                continue
            if (
                str(target_entry.get("command") or "")
                != str(source_entry.get("command") or "")
                or [str(item) for item in target_entry.get("args", [])]
                != [str(item) for item in source_entry.get("args", [])]
                or bool(target_entry.get("enabled", True))
                != bool(source_entry.get("enabled", True))
            ):
                counts["update"] += 1
                actions.append(
                    {
        ...
        return {
            ...
            "counts": counts,
            "notes": [
                "MCP 同步首版采用 merge：新增/更新 source 的 root servers，不删除目标端独有项。"
            ],
        }
    ...
    def sync_preview(
        ...
        *,
        ...
        template = (
            self._sync_skill_template(source_client, families=families)
            if include_skills
            else None
        )
        targets = []
        ...
        for target in target_clients:
            ...
            if include_skills:
                target_result["skills"] = self._target_skill_sync_preview(
                    registry, template, target
                )
                ...
                if target_result["publish_preview"]["status"] == "blocked":
                    ...
                    target_result["issues"].append(
                        "目标客户端当前 preview blocked，不能安全应用同步"
                    )
            if include_mcp:
    ...
    def _backup_sync_target_state(
        self, client: str, client_config: Dict[str, object], *, include_mcp: bool
    ) -> Tuple[str, Path]:
    ...
    def _restore_sync_target_state(
    ...
    def _apply_skill_sync_actions(
        self, registry: Dict, target_client: str, actions: List[Dict[str, object]]
    ) -> Dict[str, object]:
        applied = []
    ...
    def _apply_mcp_sync_actions(
        ...
        source_entries = [
            dict(entry) for entry in source_payload.get("root_servers", [])
        ]
        ...
        def _mutate(
            ...
            for source_entry in source_entries:
                ...
                if (
                    ...
                    != str(source_entry.get("command") or "")
                    or [str(item) for item in current.get("args", [])]
                    != [str(item) for item in source_entry.get("args", [])]
                    or bool(current.get("enabled", True))
                    != bool(source_entry.get("enabled", True))
                ):
                    ...
                    updated += 1
            return entries, "MCP merge 完成：新增 {} 个，更新 {} 个".format(
                added, updated
            )

        result = self._apply_mcp_mutation(
            target_client, "sync-merge-from-{}".format(source_client), _mutate
        )
    ...
    def sync_apply(
        ...
        *,
        ...
        if preview["blocked_targets"]:
            raise RuntimeError(
                "Refusing sync apply because targets are blocked: {}".format(
                    ", ".join(preview["blocked_targets"])
                )
            )
        ...
        try:
            ...
            for target_result in preview["targets"]:
                ...
                backup_id, backup_dir = self._backup_sync_target_state(
                    ...
                    client_config,
                    include_mcp=bool(
                        include_mcp and target_result.get("mcp", {}).get("supported")
                    ),
                )
                target_summary = {
                    "client": target_client,
                ...
                try:
                    if include_skills and target_result.get("skills"):
                        registry = self.load_registry()
                        clients = self.load_clients()
                        skill_apply = self._apply_skill_sync_actions(
                            registry, target_client, target_result["skills"]["actions"]
                        )
                        if skill_apply["changed_count"] > 0:
                            ...
                            publish_result = self.publish(
                                target_client, force=True, use_lock=False
                            )
                        else:
                            publish_result = None
                    ...
                    if include_mcp and target_result.get("mcp"):
                        if target_result["mcp"].get("supported"):
                            mcp_result = self._apply_mcp_sync_actions(
                    ...
                    self._restore_sync_target_state(
                        target_client, client_config, backup_dir
                    )
    ...
    def _extra_dir_status(
        self, client: str, client_config: Dict[str, object], registry: Dict
    ) -> List[Dict[str, object]]:
        statuses = []
        for source in self._client_source_roots(client, client_config):
            source_dir = Path(source["path"])
            discovered_dirs = (
                self.discover_skills(source_dir) if source_dir.exists() else []
            )
            discovered_fingerprints = set(
                hash_directory(skill_dir) for skill_dir in discovered_dirs
            )
            managed_fingerprints = set()
            for skill in registry["skills"].values():
                if skill.get("source_client") == client and skill.get(
                    "source_root"
                ) == str(source_dir):
                ...
                for source_record in skill.get("sources", []):
                    if source_record.get(
                        "source_client"
                    ) == client and source_record.get("source_root") == str(source_dir):
                        managed_fingerprints.add(skill.get("fingerprint"))
                        break
            statuses.append(
                {
                    ...
                    "managed": not discovered_fingerprints
                    or discovered_fingerprints.issubset(managed_fingerprints),
                }
            )
        return statuses
    ...
    def override_set(
        self, client: str, conflict_family: str, skill_id: str
    ) -> Dict[str, str]:
        registry = self.load_registry()
        ...
        if requested["conflict_family"] != conflict_family:
            raise ValueError(
                "Skill '{}' does not belong to conflict family '{}'".format(
                    skill_id, conflict_family
                )
            )
        for skill in registry["skills"].values():
            if skill["conflict_family"] == conflict_family:
                current = skill.setdefault("client_overrides", {}).get(
                    client, "inherit"
                )
                if current == "prefer" or current.startswith("prefer:"):
                    skill["client_overrides"][client] = "inherit"
        requested.setdefault("client_overrides", {})[client] = "prefer:{}".format(
            skill_id
        )
        self.save_registry(registry)
        self._refresh_statuses(registry, self.load_clients())
    ...
    def override_list(self, client: str) -> Dict[str, object]:
        ...
        for skill in sorted(
            ...
            key=lambda item: (item["conflict_family"], item["skill_id"]),
        ):
    ...
    def _family_members_visible_to_client(
        self, registry: Dict, client: str, conflict_family: str
    ) -> List[Dict[str, object]]:
        members = [
            skill
            for skill in registry["skills"].values()
    ...
    def _family_override_value(
        self, members: List[Dict[str, object]], client: str
    ) -> str:
        overrides = [
            str(member.get("client_overrides", {}).get(client, "inherit"))
            for member in members
        ]
        if overrides and all(item == "disabled" for item in overrides):
            return "disabled"
        preferred = next(
            (item for item in overrides if item.startswith("prefer:")), None
        )
        if preferred:
            return preferred
    ...
    def _set_family_override_in_registry(
        ...
        *,
        ...
        if mode == "inherit":
            ...
            if (
                requested.setdefault("client_overrides", {}).get(client)
                != preferred_value
            ):
    ...
    def batch_inherit(
        self, clients_list: List[str], families: List[str]
    ) -> Dict[str, object]:
        if not clients_list:
        ...
        for client in clients_list:
            ...
            for family in families:
                results.append(
                    self._set_family_override_in_registry(
                        registry, client=client, conflict_family=family, mode="inherit"
                    )
                )
        self.save_registry(registry)
        self._refresh_statuses(registry, clients)
    ...
    def batch_disable(
        ...
        for client in clients_list:
            ...
            for family in families:
                results.append(
                    self._set_family_override_in_registry(
                        registry, client=client, conflict_family=family, mode="disabled"
                    )
                )
        self.save_registry(registry)
        self._refresh_statuses(registry, clients)
