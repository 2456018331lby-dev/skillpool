...
def _validate_ids(
    values: list[str] | None, name: str = "identifiers"
) -> list[str] | None:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="skillpool", description="Unified skill pool manager"
    )
    parser.add_argument(
        "--root", default=None, help="Override skillpool root directory"
    )
    ...
    override_subparsers = override_parser.add_subparsers(
    ...
    inventory_parser.add_argument(
        "--skills", action="store_true", dest="inventory_skills"
    )
    ...
    scan_sources_subparsers = scan_sources_parser.add_subparsers(
        dest="scan_sources_command", required=True
    )
    ...
    scan_sources_add.add_argument(
        "--role", required=True, choices=["global_source", "client_live", "both"]
    )
    scan_sources_add.add_argument("--client", default=None)
    scan_sources_add.add_argument(
        "--kind",
        default="stable",
        choices=["stable", "workspace", "transient"],
        dest="path_kind",
    )
    ...
    discovery_subparsers = discovery_parser.add_subparsers(
        dest="discovery_command", required=False
    )
    ...
    batch_disable.add_argument(
        "--clients", action="append", dest="clients", required=True
    )
    batch_disable.add_argument(
        "--family", action="append", dest="families", required=True
    )
    batch_inherit = batch_subparsers.add_parser("inherit")
    batch_inherit.add_argument(
        "--clients", action="append", dest="clients", required=True
    )
    batch_inherit.add_argument(
        "--family", action="append", dest="families", required=True
...
def main(argv=None) -> int:
    ...
    def _cmd_import() -> int:
        sub = {
            ...
            "zip": lambda: _print(
                pool.import_zip(Path(_validate_path(args.zip_path, "zip_path")))
            ),
            "batch": lambda: _print(
                pool.import_batch(
    ...
    def _cmd_override() -> int:
        ...
        if args.override_command == "set":
            ...
            _print(
                pool.override_inherit(
                    client, _validate_id(args.conflict_family, "conflict_family")
                )
            )
        elif args.override_command == "disable":
            _print(
                pool.override_disable(
                    client, _validate_id(args.conflict_family, "conflict_family")
                )
            )
        return 0
    ...
    def _cmd_rollback() -> int:
        ...
        backup_id = (
            _validate_id(args.backup_id, "backup_id") if args.backup_id else None
        )
        if args.latest:
    ...
    def _cmd_scan_sources() -> int:
        sub = {
            ...
            "enable": lambda: _print(
                pool.scan_source_enable(_validate_id(args.id, "id"))
            ),
            "disable": lambda: _print(
                pool.scan_source_disable(_validate_id(args.id, "id"))
            ),
    ...
    def _cmd_discovery() -> int:
        sub = {
            ...
            "details": lambda: _print(
                pool.discovery_details(
    ...
    def _cmd_sync() -> int:
        ...
        sub = {
            "inspect": lambda: _print(pool.sync_inspect(source, families=families)),
            "preview": lambda: _print(
                pool.sync_preview(
                    source,
                    targets,
                    include_skills=include_skills,
    ...
    def _cmd_mcp() -> int:
        ...
        sub = {
            ...
            "enable": lambda: _print(
                pool.mcp_enable(client, _validate_id(args.server_name, "server_name"))
            ),
            "disable": lambda: _print(
                pool.mcp_disable(client, _validate_id(args.server_name, "server_name"))
            ),
            "add": lambda: _print(
                pool.mcp_add(
                    client,
            ...
            "remove": lambda: _print(
                pool.mcp_remove(client, _validate_id(args.server_name, "server_name"))
            ),
    ...
    def _cmd_mcp_update() -> int:
        ...
        _print(
            pool.mcp_update(
                client,
                srv,
                new_name=_validate_id(args.new_name, "new_name")
                if args.new_name
                else None,
                command=_validate_string(args.mcp_command_value, "command")
                if args.mcp_command_value
                else None,
                args=args.args,
                enabled=enabled,
            )
    ...
    def _cmd_serve() -> int:
        ...
        return serve(
            pool, host=args.host, port=args.port, open_browser=args.open_browser
        )
